namespace CmdLineNet.Test
{
	...
	public static class ArgsReaderTests
	{
		...
		{
			string[] args = ["-a", "alpha value", "--bravo", "bravo value", "--charlie", "charlie value", "--echo", "-f"];
		...
		{
			string[] args = ["-a", "alpha value", "--bravo", "bravo value", "lone value", "--echo", "-d", "some value", "some other value"];
		...
		{
			string[] args = ["-a", "alpha value", "--", "-a", "-b", "-c", "value"];
			Assert.Collection(Args.GetReader().Read(args),
				x => CheckArg(x, ArgId.A, "alpha value", ArgState.ShortOption),
		...
		{
			Assert.Collection(Args.GetReader().Read(["-a"]), x => CheckArg(x, ArgId.A, string.Empty, ArgState.ShortOption));
			Assert.Collection(Args.GetReader().Read(["--alpha"]), x => CheckArg(x, ArgId.A, string.Empty, ArgState.LongOption));
		}
		[Fact]
		public static void TooManyShortOptions()
		{
			Assert.Collection(Args.GetReader().Read(["-a", "blah", "-a", "blah"]),
		...
		{
			Assert.Collection(Args.GetReader().Read(["--alpha", "blah", "--alpha", "blah"]),
		...
		{
			Assert.Collection(Args.GetReader().Read(["--alpha", "blah", "-a", "blah"]),
				x => CheckArg(x, ArgId.A, "blah", ArgState.LongOption),
				x => CheckArg(x, ArgId.A, "Option was provided too many times: -a", ArgState.TooManyOptions));
		}
		[Fact]
		...
		{
			Assert.Collection(Args.GetReader().Read(["-d", "-d"]),
		...
		{
			Assert.Collection(Args.GetReader().Read(["-dd",]),
		...
		{
			Assert.Collection(Args.GetReader().Read(["-d", "--delta"]),
				x => CheckArg(x, ArgId.D, "", ArgState.ShortSwitch),
		...
		{
			Assert.Collection(Args.GetReader().Read(["--delta", "--delta"]),
		...
		public static void TooManyValues()
		{
			var r = new ArgsReaderBuilder<ArgId>()
				.Value(ArgId.V, "Value", 0, 2)
				.Build();

			Assert.Collection(r.Read(["value1", "value2", "value3"]),
		...
		{
			Assert.Collection(Args.GetReader().Read(["-x"]), x => CheckArg(x, default, "Unrecognized short argument: -x", ArgState.ShortUnrecognized));
			Assert.Collection(Args.GetReader().Read(["--randomthing"]), x => CheckArg(x, default, "Unrecognized long argument: --randomthing", ArgState.LongUnrecognized));
			Assert.Collection(Args.GetReader().Read(["-dey"]),
				x => CheckArg(x, ArgId.D, string.Empty, ArgState.StackedSwitch),
				x => CheckArg(x, ArgId.E, string.Empty, ArgState.StackedSwitch),
				x => CheckArg(x, default, "Unrecognized switch found in stacked switches: y", ArgState.ShortUnrecognized)
			);
			Assert.Collection(Args.GetReader().Read(["-dea"]),
				x => CheckArg(x, ArgId.D, string.Empty, ArgState.StackedSwitch),
				x => CheckArg(x, ArgId.E, string.Empty, ArgState.StackedSwitch),
		...
		public static void BadArgTypeEnum()
		{
			...
			{
				['o'] = new ArgMeta<ArgId>(ArgId.B, (ArgType)99999, 'o', "opt", 1, 1, null),
			}, new Dictionary<string, ArgMeta<ArgId>>()
			{
				["opt"] = new ArgMeta<ArgId>(ArgId.B, (ArgType)99999, 'o', "opt", 1, 1, null),
			}, Array.Empty<ArgMeta<ArgId>>(), [new ArgMeta<ArgId>(ArgId.B, (ArgType)99999, 'o', "opt", 1, 1, null)]);
			Assert.Collection(reader.Read(["-o"]), x => CheckArg(x, default, "ArgType enum value was not valid for argument \"-o\": 99999", ArgState.OtherError));
			Assert.Collection(reader.Read(["--opt"]), x => CheckArg(x, default, "ArgType enum value was not valid for argument \"--opt\": 99999", ArgState.OtherError));
		}
